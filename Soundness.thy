...
lemma opstep_assert[intro!]:
  \<open>p hx \<Longrightarrow>
    ...
    h' = h \<Longrightarrow>
    hx \<le> h \<Longrightarrow>
    opstep (Vis (hx, hx')) (h, Assert p) (h', Skip)\<close>
  by (clarsimp simp add: Assert_def opstep_iff weak_framed_subresource_rel_def
...
inductive safe
  ...
  where
  safe_nil[intro!]: \<open>safe [] c hl hs r g q\<close>
| safe_suc[intro]:
  \<open>\<comment> \<open> if the command is Skip, the postcondition is established \<close>
    ...
    (\<And>hs'. ma = None \<Longrightarrow> r hs hs' \<Longrightarrow> safe as c hl hs' r g q) \<Longrightarrow>
    ...
    (\<And>a c' hx'.
      ...
      safe as c' (fst hx') (snd hx') r g q) \<Longrightarrow>
    ...
    (\<And>a c' hlf hx'.
      ...
      (\<exists>hl'.
        ...
        safe as c' hl' (snd hx') r g q)) \<Longrightarrow>
    \<comment> \<open> conclude a step can be made \<close>
    safe (ma # as) c hl hs r g q\<close>

subsection \<open> Proofs about safe \<close>

inductive_cases safe_zeroE[elim!]: \<open>safe [] c hl hs r g q\<close>
inductive_cases safe_sucE[elim]: \<open>safe (ma # as) c hl hs r g q\<close>

lemma safe_nil_iff[simp]:
  \<open>safe [] c hl hs r g q \<longleftrightarrow> True\<close>
  by force

lemma safe_suc_iff:
  \<open>safe (ma # as) c hl hs r g q \<longleftrightarrow>
    (c = Skip \<longrightarrow> q (hl, hs)) \<and>
    (\<forall>hs'. ma = None \<longrightarrow>  r hs hs' \<longrightarrow> safe as c hl hs' r g q) \<and>
    (\<forall>a c' hlx hx'.
      ma = Some a \<longrightarrow>
      hl \<le> hlx \<longrightarrow>
      ((hlx, hs), c) \<midarrow>a\<rightarrow> (hx', c') \<longrightarrow>
    ...
    (\<forall>a c' hx'.
      ma = Some a \<longrightarrow>
      ((hl, hs), c) \<midarrow>a\<rightarrow> (hx', c') \<longrightarrow>
      safe as c' (fst hx') (snd hx') r g q) \<and>
    (\<forall>a c' hlf hx'.
      ma = Some a \<longrightarrow>
      ((hl + hlf, hs), c) \<midarrow>a\<rightarrow> (hx', c') \<longrightarrow>
      hl ## hlf \<longrightarrow>
      (\<exists>hl'.
        hl' ## hlf \<and> fst hx' = hl' + hlf \<and>
        (a = Tau \<longrightarrow> hl' = hl) \<and>
        safe as c' hl' (snd hx') r g q))\<close>
  apply (rule iffI)
...
lemma safe_sucD:
  \<open>safe (ma # as) c hl hs r g q \<Longrightarrow> c = Skip \<Longrightarrow> q (hl, hs)\<close>
  \<open>safe (ma # as) c hl hs r g q \<Longrightarrow> ma = None \<Longrightarrow> r hs hs' \<Longrightarrow> safe as c hl hs' r g q\<close>
  \<open>safe (ma # as) c hl hs r g q \<Longrightarrow>
    ma = Some a \<Longrightarrow>
    hl \<le> hlx \<Longrightarrow>
    a \<noteq> Tau \<Longrightarrow>
    ((hlx, hs), c) \<midarrow>a\<rightarrow> (hx', c') \<Longrightarrow>
  ...
  \<open>safe (ma # as) c hl hs r g q \<Longrightarrow>
    ma = Some a \<Longrightarrow>
    ((hl, hs), c) \<midarrow>a\<rightarrow> (hx', c') \<Longrightarrow>
    safe as c' (fst hx') (snd hx') r g q\<close>
  \<open>safe (ma # as) c hl hs r g q \<Longrightarrow>
    ma = Some a \<Longrightarrow>
    ((hl + hlf, hs), c) \<midarrow>a\<rightarrow> (hx', c') \<Longrightarrow>
    hl ## hlf \<Longrightarrow>
...
lemma safe_guarantee_monoD:
  \<open>safe as c hl hs r g q \<Longrightarrow> g \<le> g' \<Longrightarrow> safe as c hl hs r g' q\<close>
...
lemma safe_step_monoD:
  \<open>safe as c hl hs r g q \<Longrightarrow> as' \<le>\<^sub>l as \<Longrightarrow> safe as' c hl hs r g q\<close>
...
lemma safe_atom':
  \<open>sp b (wssa r p) \<le> sswa r q \<Longrightarrow>
    ...
    safe as (Atomic b) hl hs r g (sswa r q)\<close>
proof (induct as arbitrary: hl hs)
  ...
  show ?case
    ...
    apply (rename_tac hl'hlf hs' hlx hsx hlx' hsx')
...
lemma safe_seq':
  \<open>safe as c1 hl hs r g q \<Longrightarrow>
    (\<forall>as' hl' hs'. as' \<le>\<^sub>r as \<longrightarrow> q (hl', hs') \<longrightarrow> safe as' c2 hl' hs' r g q') \<Longrightarrow>
    safe as (c1 ;; c2) hl hs r g q'\<close>
proof (induct arbitrary: c2 q' rule: safe.inducts)
  case (safe_suc c1 q hl hs ma r as g)

  have c2_safe_suc:
    \<open>\<And>as' hl' hs'. as' \<le>\<^sub>r as \<Longrightarrow> q (hl', hs') \<Longrightarrow> safe as' c2 hl' hs' r g q'\<close>
    \<open>\<And>hl' hs'. q (hl', hs') \<Longrightarrow> safe (ma # as) c2 hl' hs' r g q'\<close>
    using safe_suc.prems
  ...
  show ?case
    ...
    apply (rule safe.safe_suc)
     ...
     apply (elim disjE conjE exE)
      apply (simp add: c2_safe_suc(1); fail)
     apply (force dest: c2_safe_suc(1) safe_suc.hyps(6))
      (* subgoal: local framed opstep *)
    apply (clarsimp simp add: opstep_iff simp del: sup_apply)
    apply (elim disjE conjE exE)
     apply (simp add: c2_safe_suc(1); fail)
    apply (metis c2_safe_suc(1) safe_suc.hyps(7) fst_conv snd_conv)
    done
qed force
...
lemma safe_iter:
  \<open>(\<And>as' hl' hs'. as' \<le>\<^sub>r as \<Longrightarrow> sswa r i (hl', hs') \<Longrightarrow> safe as' c hl' hs' r g (sswa r i)) \<Longrightarrow>
    sswa r i (hl, hs) \<Longrightarrow>
    safe as (Iter c) hl hs r g (sswa r i)\<close>
proof (induct as arbitrary: i hl hs rule: rev_sublisteq_list_strong_induct)
  case (ConsLess ma as)

  have safe_c2:
    \<open>\<And>as' hl' hs'. as' \<le>\<^sub>r as \<Longrightarrow> sswa r i (hl', hs') \<Longrightarrow> safe as' c hl' hs' r g (sswa r i)\<close>
    \<open>\<And>hl' hs'. sswa r i (hl', hs') \<Longrightarrow> safe (ma # as) c hl' hs' r g (sswa r i)\<close>
  ...
  show ?case
    ...
    apply (rule safe.safe_suc)
     ...
     apply clarsimp
     apply (rule safe_seq', fast intro: safe_c2(1))
     apply clarsimp
     apply (rule ConsLess.hyps, blast)
      apply (metis safe_c2(1) rev_sublisteq_trans)
     apply blast
...
lemma safe_indet:
    \<open>safe as c1 hl hs r g q \<Longrightarrow>
      safe as c2 hl hs r g q \<Longrightarrow>
      safe as (c1 \<^bold>+ c2) hl hs r g q\<close>
proof (induct as arbitrary: c1 c2 hl hs)
  case (Cons ma as)

  have safeSuc:
    \<open>safe (ma # as) c1 hl hs r g q\<close>
    \<open>safe (ma # as) c2 hl hs r g q\<close>
    using Cons.prems
    by simp+
...
lemma safe_endet:
    \<open>safe as c1 hl hs r g q \<Longrightarrow>
      safe as c2 hl hs r g q \<Longrightarrow>
      safe as (c1 \<box> c2) hl hs r g q\<close>
...
(*
     ...
     apply (clarsimp simp add: opstep_iff simp del: sup_apply)
     apply (elim disjE conjE exE)
          apply (force dest: safe_suc1(4))
         apply (force dest: safe_suc2(4))
        apply (frule opstep_tau_preserves_heap, clarsimp)

        apply (rule ConsLess.hyps, blast)
         apply (metis safe_suc1(4) fst_conv snd_conv)

    oops
        apply (fastforce intro:  dest: safe_suc1(4))
       apply (frule opstep_tau_preserves_heap, clarsimp)
...
lemma safe_frame':
  \<open>safe n c hl hs r g q \<Longrightarrow>
    ...
    safe n c (hl + hlf) hs r g (q \<^emph>\<and> sswa (r \<squnion> g) f)\<close>
proof (induct arbitrary: hlf rule: safe.induct)
  case (safe_nil c hl hs r g q)
  then show ?case by blast
next
  case (safe_suc c q hl hs ma r as g)
  ...
  show ?case
    ...
    apply (rule safe.safe_suc)
      (* subgoal: skip *)
        ...
        apply blast
      (* subgoal: rely step *)
       apply (metis safe_suc.hyps(3) sup2I1 sswa_step)
      (* subgoal: opstep guarantee *)
      apply (simp add: opstep_iff del: sup_apply)
      apply (metis act.simps(2) partial_le_part_left safe_suc.hyps(4))
      (* subgoal: plain opstep *)
     apply (frule safe_suc.hyps(7), blast, blast)
     apply (erule opstep_act_cases, force)
     apply (frule safe_suc.hyps(4)[rotated 2], force, force)
      apply (clarsimp simp del: sup_apply)
     apply (frule sswa_stepD)
      apply blast
     apply metis
      (* subgoal: local framed opstep *)
    ...
    apply (erule opstep_act_cases, force)
    apply (clarsimp simp del: sup_apply)
    apply (frule safe_suc.hyps(4)[rotated 2], blast)
      apply blast
...
(*
lemma safe_parallel':
  \<open>safe n c1 hl1 hs (r \<squnion> g2) g1 (sswa (r \<squnion> g2) q1) \<Longrightarrow>
    safe n c2 hl2 hs (r \<squnion> g1) g2 (sswa (r \<squnion> g1) q2) \<Longrightarrow>
    hl1 ## hl2 \<Longrightarrow>
    safe n (c1 \<parallel> c2) (hl1 + hl2) hs r (g1 \<squnion> g2)
      (sswa (r \<squnion> g2) q1 \<^emph>\<and> (sswa (r \<squnion> g1) q2))\<close>
...
next
  ...
  have
    \<open>\<forall>m\<le>n. safe m c1 hl1 hs (r \<squnion> g2) g1 (sswa (r \<squnion> g2) q1)\<close>
    \<open>\<forall>m\<le>n. safe m c2 hl2 hs (r \<squnion> g1) g2 (sswa (r \<squnion> g1) q2)\<close>
  ...
  then show ?case
    ...
    subgoal
      ...
      apply (elim disjE conjE exE)
       ...
       apply (frule safe_suc1(3)[rotated 2], force, force)
       apply (metis Suc.hyps safe_suc2(2) snd_conv sup2I1 sup_commute)
        (* subgoal: right *)
      ...
      apply (erule opstep_act_cases)
       apply (metis Suc.hyps fst_conv le_disj_eq_absorb snd_conv)
      apply (frule safe_suc2(3)[rotated 2], force, force)
      apply (metis Suc.hyps disjoint_sym safe_suc1(2) snd_conv sup2I1 sup_commute)
      done
    subgoal
      apply (simp add: opstep_iff del: top_apply sup_apply)
      apply (elim disjE conjE exE)
...
lemma safe_conj:
  \<open>safe as c hl hs r g q1 \<Longrightarrow>
    safe as c hl hs r g q2 \<Longrightarrow>
    safe as c hl hs r g (q1 \<sqinter> q2)\<close>
proof (induct as arbitrary: c hl hs r g q1 q2)
  ...
  show ?case
    ...
    apply (intro safe_suc conjI impI allI)
        apply blast
       apply (rule Cons.hyps; meson safe_sucD; fail)
      apply (meson safe_sucD; fail)
     apply (rule Cons.hyps; meson safe_sucD; fail)
    apply (drule safe_sucD(5), blast, blast, blast)
    apply (drule safe_sucD(5), blast, blast, blast)
    apply (case_tac a)
     apply (clarsimp simp del: inf_apply; fail)
    apply (clarsimp simp del: inf_apply)
    apply (erule opstep_act_cases)
     apply (simp add: Cons.hyps; fail)
    apply clarsimp
    apply (frule opstep_visD)
    apply (simp add: weak_framed_subresource_rel_def)
    apply (erule disjE)
     apply clarsimp
