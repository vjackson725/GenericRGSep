...
lemma safe_seq':
  \<open>safe as c1 hl hs r g q \<Longrightarrow>
    (\<forall>as' hl' hs'. as' \<le>\<^sub>l as \<longrightarrow> wssa r q (hl', hs') \<longrightarrow> safe as' c2 hl' hs' r g q') \<Longrightarrow>
    safe as (c1 ;; c2) hl hs r g q'\<close>
proof (induct arbitrary: c2 q' rule: safe.inducts)
  case (safe_suc c q hl hs ma r as g)

  have c2_safe:
    \<open>\<And>as' hl' hs'. as' \<le>\<^sub>l as \<Longrightarrow> wssa r q (hl', hs') \<Longrightarrow> safe (ma # as') c2 hl' hs' r g q'\<close>
    using safe_suc.prems
    by (clarsimp simp add: sublisteq_Cons_iff imp_ex_conjL)
  ...
  show ?case
    ...
    thm 
    apply (drule(2) c2_safe_sucD(2)[rotated 2])
thm 
      apply (simp add: safe_suc.hyps(3))
      (* subgoal: opstep guarantee *)
      apply (simp add: opstep_iff del: top_apply sup_apply)
