...
definition altsepconj :: \<open>(('a :: seplogic) \<Rightarrow> bool) \<Rightarrow> ('a \<Rightarrow> bool) \<Rightarrow> ('a \<Rightarrow> bool)\<close> where
  \<open>altsepconj P Q \<equiv> (P \<^emph> \<^bold>T) \<^bold>\<and> (P \<sim>\<^emph> Q)\<close>

lemma \<open>altsepconj P (A \<^emph> Q) \<le> A \<^emph> altsepconj P Q\<close>
proof (simp add: le_fun_def, clarsimp)
  fix h
  assume a0: \<open>altsepconj P (A \<^emph> Q) h\<close>

  obtain h1 h2
    where first_split:
      \<open>h1 \<currency> h2\<close>
      \<open>h = h1 + h2\<close>
      \<open>P h1\<close>
    using a0
    by (clarsimp simp add: altsepconj_def pred_true_def pred_conj_def sepconj_def sepcoimp_def)
  then obtain h1b h2b
    where second_split:
      \<open>h1b \<currency> h2b\<close>
      \<open>h2 = h1b + h2b\<close>
      \<open>A h1b\<close>
      \<open>Q h2b\<close>
    using a0
    by (force simp add: altsepconj_def pred_true_def pred_conj_def sepconj_def sepcoimp_def)

  show \<open>(A \<^emph> altsepconj P Q) h\<close>
    using disjoint_symm first_split second_split
    apply (simp add: altsepconj_def pred_true_def pred_conj_def sepconj_def sepcoimp_def)
    apply (rule_tac x=\<open>h1b\<close> in exI)
    apply (rule_tac x=\<open>h1 + h2b\<close> in exI)
    apply (intro conjI)
        apply force
       apply (metis partial_add_assoc partial_add_commute)
      apply force
     apply force
    apply clarsimp
    oops

lemma \<open>\<exists>sc.
  (\<lambda>p q s. sc (p s) (q s)) P (A \<^emph> Q) \<le> A \<^emph> (\<lambda>p q s. sc (p s) (q s)) P Q \<and>
  (\<lambda>p q s. sc (p s) (q s)) P (Q \<^bold>\<and> Q') = ((\<lambda>p q s. sc (p s) (q s)) P Q) \<^bold>\<and> ((\<lambda>p q s. sc (p s) (q s)) P Q')\<close>
  unfolding le_bool_def le_fun_def pred_conj_def sepconj_def
  apply (rule exI)
...
lemma altsepconj_conj_distrib_left:
  ...
  by (simp add: altsepconj_def sepcoimp_pconj_distrib_left pred_abac_eq_abc)

lemma altsepconj_disj_distrib_left:
  fixes P Q Q' :: \<open>('a :: right_cancel_seplogic) \<Rightarrow> bool\<close>
    (* precision here is not surprising, as it makes altsepconj act like sepconj *)
  assumes \<open>precise P\<close>
  shows \<open>altsepconj P (Q \<^bold>\<or> Q') = altsepconj P Q \<^bold>\<or> altsepconj P Q'\<close>
  using assms
...
lemma altsepconj_disj_distrib_left_iff_precise:
  fixes P Q Q' :: \<open>('a :: right_cancel_seplogic) \<Rightarrow> bool\<close>
  shows \<open>precise P \<longleftrightarrow> (\<forall>Q Q'. altsepconj P (Q \<^bold>\<or> Q') = altsepconj P Q \<^bold>\<or> altsepconj P Q')\<close>
  apply (rule iffI)
   apply (simp add: altsepconj_disj_distrib_left)
  apply (simp add: altsepconj_def sepconj_def pred_true_def pred_disj_def pred_conj_def sepcoimp_def
    le_fun_def precise_def)
  apply clarsimp
  apply (clarsimp simp add: le_iff_sepadd conj_disj_distribL[symmetric])
  apply (rename_tac h1a h2a)
  apply (drule_tac x=\<open>(=) h1a\<close> in spec)
  apply (drule_tac x=\<open>(=) h2a\<close> in spec)
  apply (simp add: fun_eq_iff)
  apply (drule_tac x=\<open>h1 + h1a\<close> in spec)
  apply clarsimp
  apply (erule iffE)
  apply clarsimp
  oops
...
lemma logic_prog_disj:
  assumes
    ...
    \<open>L2, I 2\<turnstile>\<^sub>c \<lbrace> P2 \<rbrace> c \<lbrace> Q2 \<rbrace>\<close>
    \<open>L1 \<inter> L2 = {}\<close>
  and invariant_altsepconj_disj_distrib_left:
    \<open>\<And>i P Q. i < length I \<Longrightarrow> altsepconj (I ! i) (P \<^bold>\<or> Q) = (altsepconj (I ! i) P) \<^bold>\<or> (altsepconj (I ! i) Q)\<close>
  shows
    \<open>L1, I 2\<turnstile>\<^sub>c \<lbrace> P1 \<^bold>\<or> P2 \<rbrace> c \<lbrace> Q1 \<^bold>\<or> Q2 \<rbrace>\<close>
proof -
  ...
  show ?thesis
    ...
    apply (intro conjI)
      ...
      apply (drule_tac x=\<open>(f lx, cc, f ly)\<close> in bspec)
       apply (force dest: bij_betwE)
      apply (case_tac cc)
           apply (force simp add: pred_disj_simp ram_comm_forward_disj)
          apply (force simp add: pred_disj_simp ram_comm_forward_disj)
         apply (force simp add: pred_disj_simp ram_comm_forward_disj)
        apply (force simp add: pred_disj_simp ram_comm_forward_disj)
       apply (fastforce simp add: invariant_altsepconj_disj_distrib_left pred_disj_simp)
      apply (fastforce simp add: invariant_altsepconj_disj_distrib_left pred_disj_simp)
     apply force
    apply (fastforce simp add: pred_disj_simp)
    done
qed

lemma logic_prog2_frame:
  assumes \<open>L, I 2\<turnstile>\<^sub>c \<lbrace> Q \<rbrace> c \<lbrace> R \<rbrace>\<close>
  shows \<open>L, I 2\<turnstile>\<^sub>c \<lbrace> P \<^emph> Q \<rbrace> c \<lbrace> P \<^emph> R \<rbrace>\<close>
proof -
  obtain G T s e
    where unfolded_triples:
      ...
      \<open>\<forall>x\<in>T. case x of
        ...
        | (l, CAcquire r, l') \<Rightarrow> r < length I \<and> altsepconj (I ! r) (G l) \<le> G l'
        | (l, CRelease r, l') \<Rightarrow> r < length I \<and> G l \<le> altsepconj (I ! r) (G l')\<close>
      \<open>Q = G s\<close>
      \<open>G e \<le> R\<close>
    using assms
    by (force simp add: logic_prog2_def)

  have 
    \<open>\<forall>x\<in>T. case x of
      (l, CRam cr', l') \<Rightarrow> ram_comm_forward cr' (P \<^emph> G l) \<le> P \<^emph> G l'
    | (l, CAcquire r, l') \<Rightarrow> r < length I \<and> altsepconj (I ! r) (P \<^emph> G l) \<le> P \<^emph> G l'
    | (l, CRelease r, l') \<Rightarrow> r < length I \<and> P \<^emph> G l \<le> altsepconj (I ! r) (P \<^emph> G l')\<close>
    ...
    apply (case_tac cr; clarsimp)
      ...
      defer
      apply (simp add: altsepconj_def)
    sorry
  then show ?thesis
    using unfolded_triples
    apply (simp add: logic_prog2_def)
